  ...
  if ($havePost) {
    ...
    if ($errors != '') {
      ...
      if ($dbOk) {
        ...
        $insQuery = "insert into actors (`last`,`first`,`date`) values(?,?,?)";
...
<?php
  if ($dbOk) {
    $query = 'select * from actors order by last';
    $result = $db->query($query);
    $numRecords = $result->num_rows;
    
    echo '<tr><th>last:</th><th>first:</th><th>date:</th><th></th></tr>';
    for ($i=0; $i < $numRecords; $i++) {
      $record = $result->fetch_assoc();
      if ($i % 2 == 0) {
        echo "\n".'<tr id="actor-' . $record['showed'] . '"><td>';
      } else {
        echo "\n".'<tr class="odd" id="actor-' . $record['showed'] . '"><td>';
      }
