  ...
  if ($havePost) {
    ...
    if ($errors != '') {
      ...
      if ($dbOk) {
        ...
        echo $firstNames . ' ' . $lastName . ', published in ' . $dob . '</div>';
...
<form id="addForm" name="addForm" action="movies.php" method="post" onsubmit="return validate(this);">
  <fieldset> 
    <div class="formData">
      ...
      <label class="field" for="firstNames">movieid:</label>
      <div class="value"><input type="text" size="60" value="<?php if($havePost && $errors != '') { echo $firstNames; } ?>" name="firstNames" id="firstNames"/></div>
      
      <label class="field" for="lastName">title:</label>
      <div class="value"><input type="text" size="60" value="<?php if($havePost && $errors != '') { echo $lastName; } ?>" name="lastName" id="lastName"/></div>
      
      <label class="field" for="dob">year:</label>
      <div class="value"><input type="text" size="10" maxlength="10" value="<?php if($havePost && $errors != '') { echo $dob; } ?>" name="dob" id="dob"/> <em>yyyy</em></div>
...
<?php
  if ($dbOk) {
    ...
    echo '<tr><th>movieid:</th><th>first:</th><th>date:</th><th></th></tr>';
